...
export function CanvasEmoji({ item, isSelected }: CanvasEmojiProps) {
  ...
  const updateEmoji = useCanvasStore(state => state.updateEmoji);
  const selectEmoji = useCanvasStore(state => state.selectEmoji);
  ...
  }, [item.id, item.x, item.y, selectEmoji]);
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isResizing) {
        ...
        updateEmoji(item.id, { scale: Math.round(newScale * 100) / 100 });
      }
      ...
      if (isRotating) {
        ...
        updateEmoji(item.id, { rotation: rotateStart.current.rotation + delta });
      }
    };
  ...
  }, [isDragging, isResizing, isRotating, item.id, updateEmoji, saveToHistory]);
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    if (e.key === 'ArrowUp') {
      e.preventDefault();
      updateEmoji(item.id, { y: item.y - moveAmount });
    } else if (e.key === 'ArrowDown') {
      e.preventDefault();
      updateEmoji(item.id, { y: item.y + moveAmount });
    } else if (e.key === 'ArrowLeft') {
      e.preventDefault();
      updateEmoji(item.id, { x: item.x - moveAmount });
    } else if (e.key === 'ArrowRight') {
      e.preventDefault();
      updateEmoji(item.id, { x: item.x + moveAmount });
    }
  }, [isSelected, item.id, item.x, item.y, updateEmoji, removeEmoji]);
